...
namespace BGUserControl
{
    ...
    [CustomExportMetadata(1, Modules.RepairSetting, "工厂模式设置", "ZZW", "1.0.0")]
    public class RepairSetting : BaseModules
    {
        ...
        RepairSettingMvvm repairSettingMvvm = new RepairSettingMvvm();
        bool? isVisible = false;
        [ImportingConstructor]
        public RepairSetting() : base(Common.controlVersion)
        {
            Loaded += RepairSetting_Loaded;
            IsVisibleChanged += RepairSetting_IsVisibleChanged;
            DataContext = repairSettingMvvm;
        }

        private void RepairSetting_IsVisibleChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
        ...
        private void InquireValueThread()
        {
            try
            {
                var CommonModelList = repairSettingMvvm.CommonSettingModelList.Where(q => q.CommonSettingType == "TextBox");
                foreach (var CommonItem in CommonModelList)
                {
                    ...
                    {
                        StartBytePositionDefaultText(CommonItem, (Convert.ToInt32(CommonItem.CommonSettingValue)).ToString());
                        continue;
                    }
        ...
        private void RepairSetting_Loaded(object sender, System.Windows.RoutedEventArgs e)
        {
        ...
        private Grid InitGrid()
        {
            ...
            foreach (CommonSettingModel CommonSetting in repairSettingMvvm.CommonSettingModelList)
            {
                int index = repairSettingMvvm.CommonSettingModelList.IndexOf(CommonSetting)  + 1;
        ...
        private Grid MakeMainGrid()
        {
            ...
            for (int i = 0; i < repairSettingMvvm.CommonSettingModelList.Count; i++)
            {
        ...
        private Grid MakeChildPanel(CommonSettingModel _csm)
        {
            ...
            if (_csm.CommonSettingType != "Button" && _csm.CommonSettingType != "ToggleButton")
            {
                ...
                BtnSendCommand.DataContext = repairSettingMvvm.CommonSettingModelList;
            }
            ///如果当前是频率项
        ...
        private void SendToggleButtonCommand(object sender, CommonSettingModel Model)
        {
            ...
            Thread.Sleep(50);
            repairSettingMvvm.SaveCommonSettingModelList();
        }
        /// <summary>
        /// ToggleButton选中事件
        /// </summary>
    ...
    public class RepairSettingMvvm : BaseMvvm
    {
        ...
        private void InitListData()
        {
            ...
            FFB.GetCarBoostSettingConfigDataModel(SystemDirectoryConfig.GetInstance().GetRepairConfig(Common.controlVersion)).ForEach(q => CommonSettingModelList.Add(q));
        }
