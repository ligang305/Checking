...
namespace BGCommunication
{
    public abstract class ConnectInterface
    {
        ...
        {
            set { cp = value; ConnInterLog = LogManager.GetLogger(cp.GetProtocolName().PadRight(25)); }
            get { return cp; }
        }
        ...
        //日志写入控制
        public ILog ConnInterLog = null;

        public void WriteError(string FunName, string Message,bool Debug = false)
        {
            string Info = FunName + "-->" + Message;
            if (Debug)
            {
                ConnInterLog?.Error(Info);
            }
        ...
        {
            WriteLog("发送", cp.FunctionName, buf, bufsize);
