...
namespace BG_Services
{
    ...
    [CustomExportMetadata(1, "BegoodBSScan", "CGN. Begood", "zhuzhiwu", "1.0.0")]
    public class BegoodBSScan : BaseScan
    {
        /// <summary>
        /// 开始扫描
        /// </summary>
        ...
        public override void BD_Scan()
        {
            ...
            Task.Run(() =>
            {
                while (true) 
                {
                    ...
                    bool isStop = IsStopScan();
                    bool isCondition = IsBSSystemCondition();
        ...
        {
            return !IsScaning  || !PLCControllerManager.GetInstance().IsConnect();
        }
        ...
        private void ReceiveStartSignal(CallBackAction _object)
        {
            ...
            var result = PLCControllerManager.GetInstance().WritePositionValue(CommandDic[Command.StartScan], true);
            ...
            do
            {
                ...
                if (IsStopScan())
                {
                    BuryingPoint($"BSEquipment Interrupt ReceiveStartSignal,Because IsScaning:{IsScaning},BSIsConnection:{PLCControllerManager.GetInstance().IsConnect()}");
                    break;
                }
