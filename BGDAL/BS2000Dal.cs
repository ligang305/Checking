...
namespace BGDAL
{
    ...
    /// 用于车速频率的控制的Dal数据处理层
    /// </summary>
    public class BS2000Dal : RepositoryFactory<BG_BS2000>
    {
        public BS2000Dal()
        {
        ...
        public List<BG_BS2000> GetBg_BsFromDataBase()
        {
            try
            {
                string sqlStr = string.Format($@"select * from BG_BS2000");
        ...
        /// <returns></returns>
        public bool DeleteBs2000(BG_BS2000 BsView)
        {
            string SqlStr = $@"delete from BG_BS2000 where ID = '{BsView.ID}' ";
            return BaseRepository().ExecuteNonQuerySQL(SqlStr) > 0;
        }
        /// <summary>
        /// 更新背散
