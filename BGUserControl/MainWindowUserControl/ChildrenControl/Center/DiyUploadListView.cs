...
namespace BGUserControl
{
    public class DiyUploadListView : BaseWindows
    {
        ...
        public DiyUploadListView()
        {
            ...
            //BegoodServerController.GetInstance().TaskV6000ArriveAction -= TaskV6000Arrive;
            //BegoodServerController.GetInstance().TaskV6000ArriveAction += TaskV6000Arrive;
        ...
        private void TaskV6000Arrive(RecvMessage recvMessage)
        {
            Application.Current.Dispatcher.InvokeAsync(() =>
            {
                MessageBox.Show("GET TaskId 1");
            });
            rm = recvMessage;
        }
        ...
        private void CommonDeleget_RequestTaskEvent()
        {
            try
            {
                ...
                {
                    //先判断Utilitis的ReceiveMessage是否为空，
                    //不为空：rm 复用，同时判断ReceiveMessageUsedTimes的值，值为1-->将Utilitis的ReceiveMessage赋值为null, 将ReceiveMessageUsedTimes赋值为0
                    //为空：GetTaskIdAndPutFTP()获取并赋值给 Utilitis的ReceiveMessage 以及rm， 将ReceiveMessageUsedTimes赋值为1
                    if (Common.ReceiveMessage == null)
                    {
                        Common.ReceiveMessage = UploadLocalImageToOnline<LocalUploadImage>.GetInstance().GetTaskIdAndPutFTP();
                        rm = Common.ReceiveMessage;
                        Common.ReceiveMessageUsedTimes = 1;
                    }
                    else
                    {
                        rm = Common.ReceiveMessage;
                        if (Common.ReceiveMessageUsedTimes == 1)
                        {
                            Common.ReceiveMessage = null;
                            Common.ReceiveMessageUsedTimes = 0;
                        }
        ...
        private void Lb_PreviewMouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            // 构造 ScanImageCache，status=0 表示这是最后（唯一）一张图
            ScanImageCache cache = new ScanImageCache()
            {
                status = 0,
                Carrid = 0,
                ImagePath = filePath
            ...
            ScanImageCacheQueue.Enqueue(cache);  // 入队，由 HandScanImageCacheThread 消费
            CommonDeleget.WriteLogAction($"手动触发上传: {filePath}", LogType.NormalLog);
            */
        }
