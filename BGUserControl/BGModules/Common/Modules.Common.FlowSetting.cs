...
namespace BGUserControl
{
    ...
    [CustomExportMetadata(1, Modules.FlowSettingModule, "阈值设置", "ZZW", "1.0.0")]
    public class FlowSetting: BaseModules
    {
        ...
        FlowSettingMvvm flowSettingMvvm = new FlowSettingMvvm();
        bool? isVisible = false;
        [ImportingConstructor]
        public FlowSetting() : base(Common.controlVersion)
        {
            Loaded += FlowSetting_Loaded;
            IsVisibleChanged += FlowSetting_IsVisibleChanged;
            DataContext = flowSettingMvvm;
        }

        private void FlowSetting_IsVisibleChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
        ...
        private void InquireValueThread()
        {
            try
            {
                var CommonModelList = flowSettingMvvm.CommonSettingModelList.Where(q => q.CommonSettingType == "TextBox");
        ...
        private void FlowSetting_Loaded(object sender, System.Windows.RoutedEventArgs e)
        {
        ...
        private Grid InitGrid()
        {
            ...
            || CurrentRole != null && CurrentRole.Equals("jjAdmin")
            || controlVersion != ControlVersion.BS)
            {
                MakeDynamicOrFixFreeze(_MainGrid);
            }

            foreach (CommonSettingModel CommonSetting in flowSettingMvvm.CommonSettingModelList)
            {
                //int indexOf = flowSettingMvvm.CommonSettingModelList.IndexOf(CommonSetting);
                //int adding = !ConfigServices.GetInstance().localConfigModel.IsLogin || (CurrentRole != null && CurrentRole.Equals("jjAdmin")) ? 2 : 1;

                int index = flowSettingMvvm.CommonSettingModelList.IndexOf(CommonSetting)  + ((!ConfigServices.GetInstance().localConfigModel.IsLogin || (CurrentRole != null && CurrentRole.Equals("jjAdmin"))) ? 2 : 1);
                if (CommonSetting.CommonSettingName.Contains("ModeSetting") && ConfigServices.GetInstance().localConfigModel.IsLogin && CurrentRole != null && !CurrentRole.Equals("jjAdmin"))
                {
        ...
        private Grid MakeMainGrid()
        {
            ...
            for (int i = 0; i < flowSettingMvvm.CommonSettingModelList.Count; i++)
            {
        ...
        private Grid MakeChildPanel(CommonSettingModel _csm)
        {
            ...
            if (_csm.CommonSettingType != "Button" && _csm.CommonSettingType != "ToggleButton")
            {
                ...
                BtnSendCommand.DataContext = flowSettingMvvm.CommonSettingModelList;
            }
            ///如果当前是频率项
        ...
        private void MakeDropDownList(CommonSettingModel _csm, Grid _dp, Label btnInner)
        {
            ...
            {
                itemSelectObject.SelectDisplayText = UpdateStatusNameAction(itemSelectObject.SelectText);
            }
            cbBox.SelectedValuePath = "SelectValue";
            cbBox.DisplayMemberPath = "SelectDisplayText";
        ...
        private void SendToggleButtonCommand(object sender, CommonSettingModel Model)
        {
            ...
            Thread.Sleep(50);
            flowSettingMvvm.SaveCommonSettingModelList();
        }
        /// <summary>
        /// ToggleButton选中事件
        /// </summary>
    ...
    public class FlowSettingMvvm : BaseMvvm
    {
        ...
        {
            FFB.SaveConfigDataModel(CommonSettingModelList,Common.controlVersion);
        }
