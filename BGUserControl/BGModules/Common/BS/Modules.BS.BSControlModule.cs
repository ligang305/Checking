        ...
        private void InitData()
        {
            ...
            switch (controlVersion) {
                case ControlVersion.BGV6000BS:
                    StatusModels = HitChModelBLL.GetInstance().GetHitchModelDataModel
                (SystemDirectoryConfig.GetInstance().GetHittingConfig(ControlVersion.BSTopCabin));
                    break;
                default:
                    StatusModels = HitChModelBLL.GetInstance().GetHitchModelDataModel
                (SystemDirectoryConfig.GetInstance().GetHittingConfig(ControlVersion.BS));
        ...
        private void GroupOnlyOne(StatusModel Item, bool StatusResult)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //{
                    //    if (int.Parse(Item.StatusIndex) > -1 && int.Parse(Item.StatusIndex) < 10) {
                    //        Item.StatusCode = StatusResult == Convert.ToBoolean(Item.DefaultValue) ? "2" : "0";
                    //    }
