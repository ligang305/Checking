...
namespace BGUserControl
{
    ...
    [CustomExportMetadata(1, Modules.FastCheck_BoostSettingModule, "加速器设置与维护", "ZZW", "1.0.0")]
    public class FastCheck_BoostSetting : BaseModules
    {
        /// <summary>
        /// 外部可能
        /// </summary>
        ...
        Car_BoostSettingBLL cbs = new Car_BoostSettingBLL(ControlVersion.FastCheck);
        [ImportingConstructor]
        public FastCheck_BoostSetting() : base(ControlVersion.FastCheck)
        {
        ...
        private void InitData()
        {
            ...
            HitChModelBLL.GetInstance().GetHitchModelDataModel
                (SystemDirectoryConfig.GetInstance().GetHittingConfig(ControlVersion.FastCheck)).Where(q => q.StatusOwner.Contains("status_Hitch_")).ToList().ForEach(q => HitchList.Add(q));
        }
        ...
        private void _BtnSendCommand_PreviewMouseUp(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            //byte StartPosition = Convert.ToByte(Position.Split('.')[0]);
            //byte EndPosition = Convert.ToByte(Position.Split('.')[1]);
            int value = Convert.ToInt32((_bd.Tag as string).Split('?')[1]);
            ...
            Task.Run(() =>
            {
                try
                {
                    PLCControllerManager.GetInstance().WritePositionValue(Position, value == 0);
            ...
            string newStr = $"{Position}?{value}";
            csm.CommonSettingDataSource = csm.CommonSettingDataSource.Replace(_bd.Tag as string, newStr);
            _bd.Tag = newStr;
            cbs.SaveConfigDataModel(CommonSettingModelList);

            _bd.Background = value == 0 ? (LinearGradientBrush)this.TryFindResource("diyBtnCarHandUnSelectColor") :
                  (LinearGradientBrush)this.TryFindResource("diyBtnCarHandSelectColor");
            _lbl.Foreground = value == 0 ? StrToBrush("#20B3F8") :
             StrToBrush("#FFFFFF");
        }
            ...
            try
            {
                ...
                else
                {
                    foreach (var HitchItem in HitchList)
                    {
                        ...
                        {
                            HitchItem.StatusCode = GlobalRetStatus[ItemIndex] ? "0" : "2";
                        }
                    }
